#include "level.h"
#include "mainwindow.h"
#include "defs.h"

#include <cmath>
#include <string>

#define SCROLLING_SPEED 3
#define DIE 0
bool check_collision(const Player& pl, const SDL_Rect& rec)
{
    //Closest point on collision box
    int cX, cY;

    //Closest x offset
    if(pl.posX < rec.x) {
        cX = rec.x;
    }
    else if( pl.posX > rec.x + rec.w ) {
        cX = rec.x + rec.w;
    }
    else {
        cX = static_cast<int>(pl.posX);
    }

    //Closest y offset
    if(pl.posY < rec.y ) {
        cY = rec.y;
    }
    else if(pl.posY > rec.y + rec.h) {
        cY = rec.y + rec.h;
    }
    else {
        cY = static_cast<int>(pl.posY);
    }

    //If the closest point is inside the circle
    if(std::hypot(cX - pl.posX, cY - pl.posY) < pl.radius) {
        return true;
    }
    else return false;
}

bool check_collision(const Player& pl, const Pipe& pip)
{
    return check_collision(pl, pip.get_lower()) || check_collision(pl, pip.get_upper());
}

Level::Level(const MainWindow& _win,const int _fl,const int _volt)
    : win(_win),
      fl(_fl),
      volt(_volt),
      background{_win.getRenderer(), BACKGROUND_LEVEL},
      pl{_win.getRenderer(), win.getWidth() / 4.0, win.getHeight() / 2.0,fl},
      jump_s{SOUND_JUMP},
      lost_s{SOUND_LOST},
      font{},
      title{_win.getRenderer()},
      pipeTexture{_win.getRenderer()},
      engine{this->seeder()},
      dist{30, 310}
{
    font.loadFromFile(FONT_PATH, 30);
    pipeTexture.loadFromFile(PIPE_IMAGE);

    for (decltype(pipes)::size_type i=0; i<pipes.size(); ++i) {
        pipes[i].init(&pipeTexture, 240);
        pipes[i].set_pos(win.getWidth() + i * (win.getWidth() + pipes[i].get_width()) / pipes.size());
        pipes[i].set_height(dist(engine));
    }
}

void Level::handle_events(const int volt)
{
    SDL_Event e;

    while (SDL_PollEvent(&e)) {
        if (e.type == SDL_QUIT) {
            next_state = GameStates::Quit;
        }
        const Uint8* currentKeyStates = SDL_GetKeyboardState(NULL);
        if (currentKeyStates[SDL_SCANCODE_UP]||e.type == SDL_MOUSEBUTTONDOWN) {
            if (!pl.isDead) {
                pl.make_jump();
                if(volt==1){jump_s.play();}
            }
        }
    }
}

void Level::logic(const int volt)
{
    // Apply changes to the player
    pl.apply_forces();
    if (!pl.isDead) {check_all_collisions(volt);
    // Check collisions
        check_bounds(volt);
    // Check score
        check_score();
    // Move forward
        background.scroll(SCROLLING_SPEED/3);
        for (auto& pip: pipes) {
            pip.scroll(SCROLLING_SPEED);
            if (pip.get_pos() < 0 - pip.get_width()) {
            // Update position and height
                pip.set_pos(win.getWidth());
                pip.set_height(dist(engine));
                pip.left_behind = false;}
        }
    }
    else{check_all_collisions(volt);
    // Check collisions

        check_bounds(volt);
    // Check score
        check_score();
    // Move forward
        background.scroll(DIE);
        for (auto& pip: pipes) {
            pip.scroll(DIE);
            if (pip.get_pos() < 0 - pip.get_width()) {
            // Update position and height
                pip.set_pos(win.getWidth());
                pip.set_height(dist(engine));
                pip.left_behind = false;}
        }
    }
}

void Level::render()
{
    win.clear();

    background.render();
    for (auto& pip: pipes) pip.render();
    title.loadFromRenderedText("Score: " + std::to_string(score), font, SDL_Color{0xFF,0,0,0xFF});

    title.render(((win.getWidth() - title.getWidth()) / 2), title.getHeight() / 2);

    pl.render();

    win.update();
}

bool Level::hit_bottom()
{
    return pl.posY + pl.radius > win.getHeight();
}

bool Level::hit_roof()
{
    return pl.posY <= pl.radius;
}

void Level::check_bounds(const int volt)
{
    if (!pl.isDead && hit_roof()) {
        pl.isDead = true;
        if(volt==1){lost_s.play();}
    }
    else if (hit_bottom()) {
        next_state = GameStates::Lost;
        if (!pl.isDead) {if(volt==1){lost_s.play();}}
    }
}

void Level::check_all_collisions(const int volt)
{
    for (auto& pip: pipes) {
        if (check_collision(pl, pip)) {
            pl.isDead = true;
            if(volt==1){lost_s.play();}
            break;
        }
    }
}

void Level::check_score()
{
    if (!pl.isDead) {
        for (auto& pip: pipes) {
            if (!pip.left_behind) {
                if (std::abs(pl.posX - (pip.get_pos() + pip.get_width())) < pl.radius) {
                    ++score;
                    pip.left_behind = true;
                    break;
                }
            }
        }
    }
}
